...
from .queries import COMMENT_REACTIONS_QUERY, CONTRIBUTORS_QUERY, ISSUES_QUERY, STARGAZERS_QUERY, RATE_LIMIT
...
def get_contributors(
    ...
    """Get repository contributors (collaborators) with their permissions."""
    variables = {"owner": owner, "name": name, "items_per_page": items_per_page}
    for page_items in _get_graphql_pages(
        access_token, CONTRIBUTORS_QUERY, variables, "collaborators", max_items
    ):
        yield map(
            lambda item: {
...
def _run_graphql_query(
    ...
    try:
        ...
        # Handle 403 errors by returning empty data (inaccessible comments/nodes)
        if e.response.status_code == 403:
            print(f"Warning: 403 Forbidden when accessing GraphQL API, skipping inaccessible resources...")
            return {}, {"cost": 0, "remaining": 0}
...
def _get_graphql_pages(
    ...
    while True:
        ...
        # If data is empty (from 403 or other errors), stop pagination
        if not data:
            print(f"Stopping pagination for {node_type} due to access error")
            break
...
def _get_comment_reaction(comment_ids: List[str], access_token: str) -> StrAny:
    ...
    for page_chunk in chunks(comment_ids, 50):
        ...
        # Skip if empty response (from 403 or other errors)
        if not page:
            print(f"Skipping {len(page_chunk)} inaccessible comments")
            continue
        print(
