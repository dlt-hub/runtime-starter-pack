...
from .helpers import get_contributors, get_reactions_data, get_rest_pages, get_stargazers
...
def github_contributors(
    ...
    """Get contributors (collaborators) in the repo `name` with owner `owner`.

    This source uses graphql to retrieve all contributors/collaborators with their permissions
    and profile information. Internally graphql is used to retrieve data. It is cost optimized
    and you are able to retrieve the data for fairly large repos quickly and cheaply.

    Args:
        owner (str): The repository owner
        name (str): The repository name
        access_token (str): The classic access token. Will be injected from secrets if not provided.
        items_per_page (int, optional): How many contributors to get in single page. Defaults to 100.
        max_items (int, optional): How many contributors to get in total. None means All.

    Returns:
        Sequence[DltResource]: One DltResource: `contributors`
    """
    return (
        dlt.resource(
            get_contributors(
